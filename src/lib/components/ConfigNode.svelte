<script lang="ts">
	...
	$: can_reset =
		type === 'enumSource'
			? JSON.stringify(node.value) !== JSON.stringify(node.defaultValue)
		...
		{#if type == 'boolean'}
			<div class="flex-1 flex gap-2 w-full justify-end">
				...
				<MinecraftButton
					flex={true}
					enabled={can_reset}
					on:click={() => (node.value = node.defaultValue)}>Reset</MinecraftButton
				>
			</div>
		{:else}
			{#if type == 'number'}
			...
			{/if}
			<MinecraftButton enabled={can_reset} on:click={() => (node.value = node.defaultValue)}
				>Reset</MinecraftButton
			>
		{/if}
	</div>
</div>
