<script lang="ts">
	...
	const load = (async () => {
		...
		tempDisc = {
			...
			'creeper-drop': {
				value: discs.every((disc) => disc['creeper-drop'] === discs[0]['creeper-drop'])
					? discs[0]['creeper-drop']
					: false,
				edited: false
			},
			lores: {
				value: discs.every((disc) => disc.lores === discs[0].lores) ? discs[0].lores : [],
				edited: false
			},
			'loot-tables': {
				value: discs.every((disc) => disc['loot-tables'] === discs[0]['loot-tables'])
					? discs[0]['loot-tables']
					: {},
				edited: false
			},
			'fragment-loot-tables': {
				value: discs.every(
					(disc) => disc['fragment-loot-tables'] === discs[0]['fragment-loot-tables']
				)
					? discs[0]['fragment-loot-tables']
					: {},
				edited: false
			},

			mono:
				uploadedDiscs.length != 0
					? {
							value: uploadedDiscs.every(
								(disc) => disc.uploadData!.mono === uploadedDiscs[0].uploadData!.mono
							)
								? uploadedDiscs[0].uploadData!.mono
								: true,
							edited: false
						}
					: undefined,
			normalize:
				uploadedDiscs.length != 0
					? {
							value: uploadedDiscs.every(
								(disc) => disc.uploadData!.normalize === uploadedDiscs[0].uploadData!.normalize
							)
								? uploadedDiscs[0].uploadData!.normalize
								: true,
							edited: false
						}
					: undefined,
			quality:
				uploadedDiscs.length != 0
					? {
							value: uploadedDiscs.every(
								(disc) => disc.uploadData!.quality === uploadedDiscs[0].uploadData!.quality
							)
								? uploadedDiscs[0].uploadData!.quality
								: 'high',
							edited: false
						}
					: undefined,
			...
			texture: {
				value: multiple
					...
					: discs[0].packData
						? discs[0].packData.texture
						: discs[0].uploadData!.uploadedTexture,
				edited: false
			},
			...
			fragment_texture: {
				value: multiple
					...
					: discs[0].packData
						? discs[0].packData.fragmentTexture
						: discs[0].uploadData!.uploadedFragmentTexture,
				edited: false
			}
		};
	...
	{:then _}
		...
		<div class="flex flex-col justify-between gap-4 sm:flex-row">
			<div class="flex gap-2 justify-around">
				<img
					class="bg-[#202020] p-3 cursor-pointer h-28 sm:h-32 h-max-32 aspect-square border border-transparent hover:border-white"
					src={URL.createObjectURL(tempDisc.texture.value)}
					alt="disc icon"
					use:inputFile={{
						accept: 'image/*',
						cb: setTexture
				...
				/>
				<img
					class="bg-[#202020] p-3 cursor-pointer h-28 sm:h-32 aspect-square border border-transparent hover:border-white"
					src={URL.createObjectURL(tempDisc.fragment_texture.value)}
					alt="disc icon"
					use:inputFile={{
						accept: 'image/*',
			...
			<div class="flex flex-col flex-1 justify-around gap-2">
				...
				<MinecraftTextbox
					placeholder="Author"
					bind:value={tempDisc.author.value}
		...
		<FormatEditor
			on:input={() => setUpdatedPropery('lores')}
			bind:split={tempDisc.lores.value}
			text={initalText}
			firstline={tempDisc.author.value != ''
				? `${tempDisc.title.value} - ${tempDisc.author.value}`
				: tempDisc.title.value}
		/>
		...
		<div class="flex flex-col justify-around gap-4 sm:flex-row items-center mt-4">
			<div class="flex justify-start gap-4 flex-row items-center">
				<MinecraftCheckbox
					onClick={() => setUpdatedPropery('creeper-drop')}
					bind:value={tempDisc['creeper-drop'].value}
					size="0px"
				/>
		...
		{#if tempDisc.mono && tempDisc.normalize && tempDisc.quality}
			...
			<div class="flex flex-col justify-around gap-4 items-center mt-4 w-full">
				<div class="flex justify-start gap-4 flex-row items-center">
					<MinecraftCheckbox
						bind:value={tempDisc.mono.value}
						size="0px"
						onClick={() => setUpdatedPropery('mono')}
					/>
					<h5 class="h5 font-minecraft h-min">Convert to mono</h5>
				</div>
				<div class="flex justify-start gap-4 flex-row items-center">
					<MinecraftCheckbox
						bind:value={tempDisc.normalize.value}
						size="0px"
						onClick={() => setUpdatedPropery('normalize')}
					/>
				...
				<div class="flex justify-start gap-4 flex-row items-center w-[30%]">
					...
					<MinecraftComboBox
						...
						bind:value={tempDisc.quality.value}
						on:input={() => setUpdatedPropery('quality')}
					/>
				</div>
			</div>
		{/if}
		...
		<div class="flex w-full items-center justify-center mt-4 gap-4">
			...
			<button class="btn variant-filled-error text-white" on:click={deleteDisc}
				>Delete disc{multiple ? 's' : ''}</button
			>
		</div>
	{/await}
</main>
